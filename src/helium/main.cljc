(ns helium.main
  "Kotodama WASM entrypoint for the Helium clean-room actor (L4) — Clojure port.
  ...
  facts (`helium.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "helium")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"helium.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "helium-compat" :tier tier :entities entities} 200])
...
(defn start! [] :helium-compat/ready)
